...
def streams(streams_gdf: str,
    ...
    adjoint_dict = create_adjoint_json(streams_gdf, id_field=id_field, ds_field=ds_field, order_field="strmOrder")
    ...
    if 0 in streams_gdf[len_field].values:
        logger.info("Classifying length 0 segments")
        zero_length_fixes_df = identify_0_length(streams_gdf, id_field, ds_field, len_field)
    ...
    adjoint_order_2_dict = create_adjoint_json(streams_gdf, id_field=id_field, ds_field=ds_field,
    ...
    order_1_branches = streams_gdf.loc[streams_gdf[order_field] == 1]
    higher_order_trunks = streams_gdf.loc[streams_gdf[order_field] >= 3, id_field].values
    downstream_connections = order_1_branches.loc[order_1_branches[ds_field].isin(higher_order_trunks), ds_field]
    pairs_to_prune = streams_gdf.loc[streams_gdf[id_field].isin(downstream_connections), ['USLINKNO1', 'USLINKNO2']]
    pairs_to_prune['orderUS1'] = pairs_to_prune['USLINKNO1'].apply(
        lambda x: streams_gdf.loc[streams_gdf['LINKNO'] == x, 'strmOrder'].values[0])
